...
Pandora is a **Cogni full-app submodule node**. This repo owns the node app,
graphs, package closure, review policy, and node CI. The operator monorepo
(`Cogni-DAO/cogni`) pins this repo as `nodes/pandora/` and owns deployment
overlays, catalog entries, Argo, DNS, and candidate/preview/production flights.

Seeded from `Cogni-DAO/cogni:nodes/node-template/` and projected to a
node-at-repo-root build surface.

- **Node-at-root** layout (`app/`, `graphs/`, `packages/`, `.cogni/`) so it mounts cleanly at `nodes/pandora/` when added as a submodule.
...
pnpm check
docker build --target runner -t pandora:local .
```
